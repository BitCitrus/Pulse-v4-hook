...
| 费率刷新 | 初始化时使用 `MIN_FEE`，此后每个新区块的第一笔交换重新计算，合约可见 `block.number` 相同的后续交换复用缓存费率。 |
...
精确输入交换从输出币收取，精确输出交换从输入币收取。没有小费（`tx.gasprice == block.basefee`）时，费率为 **1 bp**。小费等于 base fee 时，总费率为 **2 bp**。Hook 协议费总上限为 **30 bp / 0.30%**。暂停收费或 `block.basefee == 0` 时，这笔附加费为零。

按示例配置，LP 费率最高为 30 bp，两项标示费率合计最高为 **60 bp**。实际扣费分别按各自的计费金额计算和取整。
...
Hook 回归测试为 **81 项通过，0 失败、0 跳过**。Hook 部署后的字节码为 **8,046 字节**，低于 24,576 字节上限。测试覆盖原生币结算、claim 与收入账本的一致性、提现回滚、带符号金额边界，以及三个共享币种的池中执行 300 次交换后提取收入的完整流程。部署测试覆盖共享配置、价格换算、整数边界和 CREATE2 计划；可选的 [Robinhood fork 测试](docs/robinhood.md)覆盖正式 Universal Router、PositionManager、Permit2 和含协议费的滑点校验。
...
从 [`.env.example`](.env.example) 和[部署指南](docs/launch.md)开始，填写目标链的 PoolManager、管理员与代币地址，并核对初始价格和费率参数。预检与部署使用同一配置解析器；`INITIAL_SQRT_PRICE` 留空时，两者都从 `PRICE_E18` 推导池价。自动推导支持 0–18 位小数；其他格式需直接提供排序后的池价。

部署脚本通过 CREATE2 搜索 Hook 地址并初始化池子。地址的**最低 14 位必须满足权限掩码 `0x10C4`**，池子必须使用动态费用标志 **`0x800000`**。流动性需在初始化后另外添加。Robinhood 上费用缓存按估算的 L1 块号更新，成交量按时间衰减。普通交易在 `gasprice == basefee` 时收 1 bp 协议费，仍保留按比例计算的公式。Uniswap 前端路由接入需要另行完成 [Hook 审核](https://support.uniswap.org/hc/en-us/articles/48291859140621-Routing-for-hooked-pools)。
