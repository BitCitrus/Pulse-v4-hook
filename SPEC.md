...
即**每区块第一笔交易重算,同区块内后续交易使用同一费率**。没有外部刷新入口,
没有 keeper,也没有基于时间的过期窗口——任何人都无法决定别人付什么费率,
也无法在同一区块内先改变成交量信号再据此交易。
...
否则:
    extraPips = min(3000, floor(tx.gasprice × 100 / block.basefee))
    totalPips = 100 + extraPips
    hookFee = floor(abs(unspecifiedDelta) × totalPips / 1_000_000)
```

总上限为 **3100 pips / 31bp / 0.31%**。gas price 与 base fee 相同时,费率为 200 pips / 0.02%。
...
- 费率对"窗口占全局成交量的比例"是**连续单调**的,`local == 0` 只是该曲线的极限,不是跳变;
  但当窗口只占全局成交量的一小部分时费率就已经贴上 `MAX_FEE`(当前参数下约 1%),
