...
| [lib/VolumeDecayLib.sol](../src/lib/VolumeDecayLib.sol) | 纯函数:整小时 `0.8^h` 衰减,Q96 二进制乘方 |
...
**每区块刷新而非定时缓存。** 早期版本用 120 秒过期窗口加公开 `pokeFee`(60 秒冷却)。
那种设计下,**先调用 `pokeFee` 的人决定了随后 120 秒内所有交易者支付的费率**。
改成每区块首笔自动重算后,这个窗口归零,`pokeFee` 也随之失去存在意义并被删除。
代价是每区块首笔交易多付约 12k gas 的重算成本。

**暂停不阻断交易。** `afterSwap` 中 revert 会瘫痪整个池子。暂停因此只让协议费归零。

**不实现 `beforeSwapReturnDelta`。** 协议费从 `afterSwap` 的 `hookDeltaUnspecified` 收取,
这样 core 的交换数学完全不受 Hook 影响。
