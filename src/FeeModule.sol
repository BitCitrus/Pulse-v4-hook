...
abstract contract FeeModule is HookBase {
    ...
    /// @notice Block in which cachedFee was last recomputed. Caching per block (rather than
    ///         for a wall-clock window) keeps a multi-swap block cheap while ensuring no party
    ///         can pin the fee every other trader pays beyond the block they are in.
    ...
    /// @dev Fee used by a swap: recomputed on the first swap of each block and reused by any
    ///      later swap in the same block. Per-block caching keeps a busy block cheap while
    ///      ensuring nobody can pin the fee that other traders pay beyond their own block.
